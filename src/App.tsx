  ...
  React.useEffect(() => {
    try {
      ...
      if (!metaTheme) {
        ...
        dbg("Added theme-color meta:", m.content);
      } else {
        dbg("Existing theme-color meta found:", metaTheme.content);
      }
    } catch (err) {
    ...
    <div className="relative min-h-screen w-full overflow-hidden text-white">
      ...
      <main className="relative z-10 mx-auto flex min-h-screen max-w-5xl flex-col items-center justify-center px-6 py-16">
        <motion.div variants={stagger} initial="hidden" animate="show" className="w-full">
          <motion.div variants={floating} className="mx-auto mb-6 flex items-center justify-center">
            {!logoBroken ? (
              <img
          ...
          {/* Interested CTA (reveals the form) */}
          {!interestOpen && (
...
function TeaserPill({ label, color }: { label: string; color: string }) {
  return (
    <span
      ...
      onMouseEnter={() => dbg("Pill hover:", label)}
    >
      {label}
    </span>
  );
